...
const KidsPage: React.FC<KidsPageProps> = (props: KidsPageProps) => {
    ...
    const [option, setOption] = useState<string>('kids');
    ...
    const handleSetOption = (option: string) => {
        ...
        setOption(option);
        router.push('/kids?page=1');
    };

    const { data, isLoading, isSuccess, isError } = useQuery({
        queryKey: ['getProducts', page, option],
        queryFn: async () => await getProductsByOption(page, PAGE_SIZE, option),
    });
